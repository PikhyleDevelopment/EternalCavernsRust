...
enum BuildingTag {
    Pub, Temple, Blacksmith, Clothier, Alchemist,
    PlayerHouse, Hovel, Abandoned, Unassigned
}
...
impl TownBuilder {
    ...
    fn building_factory(&mut self,
			...
			buildings_index: &[(usize, i32, BuildingTag)])
    {
	for (i, building) in buildings.iter().enumerate() {
	    let build_type = &buildings_index[i].2;
	    match build_type {
